...
class DPTViTAttention(nn.Cell):
    ...
    # Copied from mindone.transformers.models.t5.modeling_t5.T5Attention.prune_linear_layer
    ...
    # Copied from mindone.transformers.models.t5.modeling_t5.T5Attention.find_pruneable_heads_and_indices
    ...
    def prune_heads(self, heads: Set[int]) -> None:
        ...
        # Prune linear layers
        self.attention.query = self.prune_linear_layer(self.attention.query, index)
        self.attention.key = self.prune_linear_layer(self.attention.key, index)
        self.attention.value = self.prune_linear_layer(self.attention.value, index)
        ...
        for i, hidden_state in enumerate(hidden_states):
            if i not in self.neck_ignore_stages:
                ...
                if self.config.readout_type == "project":
                    ...
                    hidden_state = self.readout_projects[i](ops.cat((hidden_state, readout), axis=-1))
...
class DPTFeatureFusionLayer(nn.Cell):
    ...
    def construct(self, hidden_state, residual=None):
        ...
        hidden_state = ops.interpolate(hidden_state, scale_factor=2, mode="bilinear", align_corners=self.align_corners)
        hidden_state = self.projection(hidden_state)
...
class DPTDepthEstimationHead(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.head = nn.SequentialCell(
            nn.Conv2d(features, features // 2, kernel_size=3, stride=1, padding=1, pad_mode="pad"),
            nn.Upsample(scale_factor=2, mode="bilinear", align_corners=True),
            nn.Conv2d(features // 2, 32, kernel_size=3, stride=1, padding=1, pad_mode="pad"),
            nn.ReLU(),
    ...
    def construct(
        ...
        if self.backbone is not None:
            ...
            if not self.config.is_hybrid:
                ...
                backbone_hidden_states = list(outputs[-1])
