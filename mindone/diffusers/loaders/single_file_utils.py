...
def update_vae_attentions_ldm_to_diffusers(keys, new_checkpoint, checkpoint, mapping):
    for ldm_key in keys:
        ...
        if len(shape) == 3:
            new_checkpoint[diffusers_key] = Parameter(new_checkpoint[diffusers_key][:, :, 0])
        elif len(shape) == 4:
            new_checkpoint[diffusers_key] = Parameter(new_checkpoint[diffusers_key][:, :, 0, 0])
...
def create_text_encoder_from_open_clip_checkpoint(
    ...
    **config_kwargs,
    ...
    text_model_dict["text_model.embeddings.position_ids"] = Parameter(text_model.text_model.embeddings.position_ids)
    ...
    for diffusers_key, ldm_key in openclip_diffusers_ldm_map.items():
        ...
        if ldm_key.endswith("text_projection"):
            text_model_dict[diffusers_key] = Parameter(checkpoint[ldm_key].T.contiguous())
        else:
    ...
    for key in keys:
        ...
        if key.endswith(".in_proj_weight"):
            ...
            weight_value = checkpoint[key]
            text_model_dict[diffusers_key + ".q_proj.bias"] = Parameter(weight_value[:text_proj_dim])
            text_model_dict[diffusers_key + ".k_proj.bias"] = Parameter(weight_value[text_proj_dim : text_proj_dim * 2])
            text_model_dict[diffusers_key + ".v_proj.bias"] = Parameter(weight_value[text_proj_dim * 2 :])
        else:
            text_model_dict[diffusers_key] = checkpoint[key]
