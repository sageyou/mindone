...
class StableDiffusionLatentUpscalePipeline(DiffusionPipeline, FromSingleFileMixin):
    ...
    def __call__(
        ...
        >>> upscaler = StableDiffusionLatentUpscalePipeline.from_pretrained(model_id, torch_dtype=ms.float16)
        >>> upscaler.to("cuda")
        ...
        noise_level = ms.Tensor([0.0], dtype=ms.float32)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                scaled_model_input = ops.cat([scaled_model_input, image_cond], dim=1)
